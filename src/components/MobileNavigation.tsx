...
export default function MobileNavigation() {
  ...
  const navigationItems = [
    ...
    { to: '/circulars', icon: LinkIcon, label: language === 'hindi' ? 'परिपत्र' : 'Circulars', key: 'circulars' },
    { to: '/document', icon: FileText, label: language === 'hindi' ? 'दस्तावेज़' : 'Document', key: 'document' },
    { to: '/academy', icon: GraduationCap, label: language === 'hindi' ? 'अकादमी' : 'Academy', key: 'academy' },
    { to: '/glossary', icon: BookOpen, label: language === 'hindi' ? 'शब्दकोश' : 'Glossary', key: 'glossary' },
    { to: '/videos', icon: PlayCircle, label: language === 'hindi' ? 'वीडियो' : 'Videos', key: 'videos' }
  ];

  const currentPath = location.pathname;
  
  // Show only 3 main items when collapsed
  const visibleItems = isExpanded ? navigationItems : navigationItems.slice(0, 3);
  const hiddenCount = navigationItems.length - 3;

  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 lg:hidden">
      {/* Navigation Items */}
      <div className={`transition-all duration-300 ease-in-out ${isExpanded ? 'pb-12' : 'pb-2'}`}>
        <div className={`grid gap-1 p-2 max-w-md mx-auto transition-all duration-300 ${
